...
app.controller('NewFlightController', ['$scope', '$rootScope', '$location', '$resource', '$http','socket',
    ...
    $scope.removeChoice = function(index,parse) {
     ...
     $scope.registerFlight = function() {
       ...
       if ($scope.missing_fields.length > 0){
         $scope.errorMsg = "Missing Fields: " + $scope.missing_fields;
         $('#errorModal').modal();
         return;
       }
       ...
       xhttp.onreadystatechange = function() {
        ...
        if (this.readyState === 4 && this.status === 400) {
          $scope.errorMsg = "There was an error in registering the flight: " + JSON.parse(this.responseText);
          $('#errorModal').modal();
          return;
        }
