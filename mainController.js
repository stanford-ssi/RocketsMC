...
('MainController',
 ['$scope',
  ...
  function ($scope, $rootScope, $location, $resource, $http, socket) {
      ...
      $scope.main.subscribed = true; //true when the user is on the subscribers list
      $scope.main.Series = {};
	 ...
	 in the following sctuct, indexed by flight name */
	  ...
	  for(var i = 0; i < $scope.main.num_channels[launch_name]; i++){
	      $scope.main.telemData[launch_name][i] = [];
	      $scope.main.graphData[launch_name][i] = [[]];
	  ...
	  if ($scope.main.graphData[launch_name].timeStamps.length > iterate_val) {
	      $scope.main.graphData[launch_name].timeStamps.splice(0,1);
	      for(i = 0; i < $scope.main.num_channels[launch_name]; i++) {
		  $scope.main.graphData[launch_name][i][0].splice(0,1);
	      }
	  }
      };
      ...
      socket.on('from:kythera', function(str_message){
	  var message = JSON.parse(str_message);
	  //console.log("Message from Kythera: " + str_message);
	  ...
	  if(data_list[1] === "D"){
	      ...
	      for(var g = 0; g < $scope.main.num_channels[$scope.main.selected_launch_name]; g++){
		  checksum += last_sum;
		  $scope.main.telemData[message.launch_name][g].push(parseFloat(data_list[g+2]));
		  last_sum = parseInt(data_list[g+2]);
	      }
	  ...
	  $scope.$apply( function(){
		  ...
		  $scope.main.liveViewInfo[$scope.main.selected_launch_name] &&
		  $scope.main.liveViewInfo[$scope.main.selected_launch_name].last_packet_time !== null) {
      ...
      $scope.main.controlFlight = function(flight){
	  $scope.main.flightToControl = flight;
	  //socket.emit("subscribeToDownlink");
